...
class OrderAuthorizationServiceTest {
    ...
    private final OrderAuthorizationService service = new OrderAuthorizationService(
            orderRepository,
            mock(ReturnOrderRepository.class),
            mock(RefundRequestRepository.class));

    @Test
    void allowsOwnerAndRejectsAnotherCustomer() {
        when(orderRepository.findById("order-1"))
                .thenReturn(Optional.of(Order.builder().customerId("user-1").build()));

        service.requireOrderAccess("order-1", authentication("user-1", "ROLE_CUSTOMER"));

        assertThatThrownBy(() -> service.requireOrderAccess(
                "order-1", authentication("user-2", "ROLE_CUSTOMER")))
