...
class OrderControllerPaymentTest {
    ...
    void setUp() {
        ...
        controller = new OrderController(orderService, mock(DashboardService.class), mock(OrderAuthorizationService.class), verifier, mock(PaymentConfigurationValidator.class));
    }
    ...
    void vnpayReturnsPaidStatusOnlyAfterBackendProcessesBothSuccessCodes() {
        ...
        when(orderService.processVnpayIpn(anyString(), anyString(), anyString(), anyLong(), anyString())).thenReturn(new PaymentProcessingResult(Order.PaymentStatus.PAID, false));
        var response = controller.vnpayIpn(payload);
        assertThat(body(response.getBody())).containsEntry("RspCode", "00").containsEntry("paymentStatus", "PAID");
    }
    ...
    void vnpayMapsAmountMismatchToOfficialResponseCode() {
        Map<String, String> payload = validVnpayPayload();
        when(verifier.verifyVnpay(payload)).thenReturn(true);
        when(orderService.processVnpayIpn(anyString(), anyString(), anyString(), anyLong(), anyString())).thenThrow(new ResponseStatusException(HttpStatus.BAD_REQUEST, "Số tiền thanh toán không khớp đơn hàng"));
        var response = controller.vnpayIpn(payload);
    ...
    void momoBrowserReturnUsesPersistedPaymentStatus() {
        ...
        when(orderService.processMomoIpn("ORD-1", 0, 130_000, "MOMO-TXN-1")).thenReturn(new PaymentProcessingResult(Order.PaymentStatus.PAID, false));
        var response = controller.momoReturn(payload);
        assertThat(body(response.getBody())).containsEntry("paymentStatus", "PAID").containsEntry("confirmed", true);
    }
