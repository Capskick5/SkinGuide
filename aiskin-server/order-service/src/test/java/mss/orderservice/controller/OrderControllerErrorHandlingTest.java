...
class OrderControllerErrorHandlingTest {
    ...
    void unexpectedCancellationFailureIsDelegatedToGlobalHandler() {
        ...
        when(orderService.cancelOrder("order-1", "Changed my mind"))
                .thenThrow(new RuntimeException("mongodb://database.internal:27017"));

        assertThatThrownBy(() -> controller.cancelOrder(
                "order-1", Map.of("cancelReason", "Changed my mind"), authentication))
                .isInstanceOf(RuntimeException.class)
                .hasMessageContaining("database.internal");
    }

    @Test
    void unexpectedGhnSyncFailureIsDelegatedToGlobalHandler() {
        org.mockito.Mockito.doThrow(new RuntimeException("carrier-secret-detail"))
                .when(orderService).syncGhnOrderStatus();

        assertThatThrownBy(controller::syncGhnOrderStatusManual)
                .isInstanceOf(RuntimeException.class)
                .hasMessageContaining("carrier-secret-detail");
    }
}
