...
class DemoInventoryServiceTest {
    ...
    private final IInventoryService inventoryService = mock(InventoryService.class);

    private final IDemoInventoryService service = new DemoInventoryService(productRepository, inventoryService);

    @Test
    void initializesOnlyEmptyTrackedVariantsUsingReceiptMovement() {
        ProductVariant empty = ProductVariant.builder().id("variant-empty").trackInventory(true).inventoryLevels(List.of()).build();
        ProductVariant stocked = ProductVariant.builder().id("variant-stocked").trackInventory(true).inventoryLevels(List.of(InventoryLevel.builder().warehouseId(InventoryService.DEFAULT_WAREHOUSE_ID).onHandQuantity(5).build())).build();
        when(productRepository.findAll()).thenReturn(List.of(Product.builder().id("product-1").variants(List.of(empty, stocked)).build()));
        IDemoInventoryService.SeedInventoryResult result = service.seedMissingInventory(50);
    ...
    void initializesSyntheticVariantForLegacyProduct() {
        when(productRepository.findAll()).thenReturn(List.of(Product.builder().id("legacy-product").slug("legacy-product").price(120_000D).isActive(true).build()));
        IDemoInventoryService.SeedInventoryResult result = service.seedMissingInventory(25);
        assertThat(result.initializedVariants()).isEqualTo(1);
        ArgumentCaptor<InventoryAdjustmentRequest> captor = ArgumentCaptor.forClass(InventoryAdjustmentRequest.class);
        verify(inventoryService).adjust(captor.capture());
