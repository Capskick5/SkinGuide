...
public class DemoInventoryService {
    ...
    public SeedInventoryResult seedMissingInventory(int quantityPerVariant) {
        ...
        for (Product product : productRepository.findAll()) {
            for (ProductVariant variant : variants(product)) {
                ...
                inventoryService.adjust(InventoryAdjustmentRequest.builder()
                        .productId(product.getId())
                        .variantId(variant.getId())
                        .warehouseId(InventoryService.DEFAULT_WAREHOUSE_ID)
                        .operationType(InventoryAdjustmentRequest.OperationType.RECEIPT)
                        .quantityDelta(quantityPerVariant)
                        .reason("Nhập tồn kho demo ban đầu")
                        .build());
                initialized++;
            }
        }
    ...
    private boolean hasMainWarehouseStock(ProductVariant variant) {
        ...
        return variant.getInventoryLevels().stream()
                .filter(level -> InventoryService.DEFAULT_WAREHOUSE_ID.equals(level.getWarehouseId()))
                .map(InventoryLevel::getOnHandQuantity)
                .anyMatch(quantity -> quantity != null && quantity > 0);
    }

    public record SeedInventoryResult(
            int initializedVariants,
            int skippedVariants,
            int totalVariants,
            int quantityPerVariant) {
    }
}
