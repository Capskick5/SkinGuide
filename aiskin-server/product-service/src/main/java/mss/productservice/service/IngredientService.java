...
public class IngredientService {
    ...
    public List<IngredientResponse> getIngredientsByConcern(String concern) {
        return ingredientRepository.findByConcernsContaining(concern).stream()
                .map(this::toResponse)
                .toList();
    }

    public List<IngredientResponse> getIngredientsByBenefit(String benefit) {
        return ingredientRepository.findByBenefitsContaining(benefit).stream()
                .map(this::toResponse)
                .toList();
    }

    public List<IngredientResponse> getSafeIngredients(Integer maxEwgScore) {
        return ingredientRepository.findByEwgScoreLessThanEqual(maxEwgScore).stream()
                .map(this::toResponse)
                .toList();
    }
    ...
    private IngredientResponse toResponse(Ingredient ingredient) {
        return IngredientResponse.builder()
                ...
                .aliases(ingredient.getAliases())
                .description(ingredient.getDescription())
                .benefits(ingredient.getBenefits())
                .concerns(ingredient.getConcerns())
                .contraindications(ingredient.getContraindications())
                .ewgScore(ingredient.getEwgScore())
